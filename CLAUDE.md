...
- [x] **Step 2 — Local data model, durable event log, financial calculation.** `AlarmDefinition` (validated at construction: rate/max must be positive, currency code well-formed — no way to represent "unlimited" charge), `AlarmSession` + `AlarmSessionEvent` (append-only, idempotent against duplicate/out-of-order/late events — important since step 3 will have two processes, the app and the AlarmKit intent extension, both able to write), `MoneyCalculator.amountOwed` (pure function: sums snoozeStarted→dismissed intervals, clamped to `maxChargeAmount`, `Decimal`-based to avoid float error, rounded only at the final currency amount), `AlarmSessionStore` (atomic file-per-session JSON persistence — crash mid-write can never corrupt or half-write a session). Full XCTest coverage written; **not yet run** (no Mac in this session — first thing to verify once CI is live).
- [ ] **Step 3 — AlarmKit integration (no billing yet).** Schedule/fire, Stop + Snooze intents, widget extension + App Group entitlement, manual on-device verification matrix (locked/backgrounded/force-quit/reboot/Focus/Silent). This is where the theoretical AlarmKit understanding from the investigation gets checked against real device behavior — expect corrections to this log once that happens.
